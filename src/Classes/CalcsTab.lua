		...
		for nodeId, node in pairs(nodes) do
			if not node.alloc and node.modKey ~= "" and not self.mainEnv.grantedPassives[nodeId] then
				if not cache[node.modKey] then
					cache[node.modKey] = calcFunc({ addNodes = { [node] = true } }, useFullDPS, self.powerStat)
				end
				...
				if self.powerStat and self.powerStat.stat and not self.powerStat.ignoreForNodes then
					...
					if node.path and not node.ascendancyName then
						...
						if node.pathDist > 1 then
							node.power.pathPower = self:CalculatePowerStat(self.powerStat, calcFunc({ addNodes = pathNodes }, useFullDPS, self.powerStat), calcBase)
						end
					end
			...
			elseif node.alloc and node.modKey ~= "" and not self.mainEnv.grantedPassives[nodeId] then
				if not cache[node.modKey.."_remove"] then
					cache[node.modKey.."_remove"] = calcFunc({ removeNodes = { [node] = true } }, useFullDPS, self.powerStat)
				end
				...
				if self.powerStat and self.powerStat.stat and not self.powerStat.ignoreForNodes then
					...
					if node.depends and not node.ascendancyName then
						...
						if #node.depends > 1 then
							node.power.pathPower = self:CalculatePowerStat(self.powerStat, calcFunc({ removeNodes = pathNodes }, useFullDPS, self.powerStat), calcBase)
						end
					end
				end
			end
